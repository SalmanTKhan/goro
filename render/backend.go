...
func wireInput(events gpucontext.EventSource, state *input.State, optional ...any) {
	...
	for _, value := range optional {
		...
		case func() bool:
			synthetic = callback
		case func(input.InputSource):
			sourceChanged = append(sourceChanged, callback)
		}
	}
...
func uiTextBoxPosition(screen *Frame, box UITextBoxCommand, cached cachedOverlayImage) (float64, float64) {
	...
	default:
		...
		x := box.X - float64(cached.width)/2
		y := box.Y
		x = clampOverlayFloat64(x, 0, maxOverlayFloat64(0, float64(screenW-cached.width)))
		y = clampOverlayFloat64(y, 0, maxOverlayFloat64(0, float64(screenH-cached.height)))
		return x, y
	}
}
...
func overlayTextBoxSize(canvas widget.Canvas, lines []string, style overlayTextBoxStyle) (int, int) {
	width := int(math.Ceil(float64(style.minWidth)))
	for _, line := range lines {
		lineWidth := int(math.Ceil(float64(rotheme.MeasureText(canvas, line, style.size, style.bold))))
		if lineWidth > width {
			width = lineWidth
		}
	}
	width += int(math.Ceil(float64(style.padX * 2)))
	if max := int(math.Ceil(float64(style.maxWidth))); max > 0 && width > max {
		width = max
	}
	height := int(math.Ceil(float64(style.padY*2 + style.lineH*float32(len(lines)))))
	return width, height
}
...
func bannerOverlayTextBoxStyle(size float32, bold bool, foreground color.RGBA, maxWidth float32) overlayTextBoxStyle {
	...
	return overlayTextBoxStyle{
		size: size, lineH: size + 4, padX: 10, padY: 6,
		maxWidth: maxWidth, maxLines: 1, wrap: true, bold: bold,
...
func wrapOverlayText(canvas widget.Canvas, text string, size float32, bold bool, maxWidth float32) []string {
	...
	for _, word := range words {
		if line == "" && rotheme.MeasureText(canvas, word, size, bold) > maxWidth {
			part := ""
			for _, r := range word {
				candidate := part + string(r)
				if part != "" && rotheme.MeasureText(canvas, candidate, size, bold) > maxWidth {
