...
func (m *WorldMode) DrawInventoryItemIconSized(screen *render.Frame, manager *res.Manager, item session.InventoryItem, x, y, size int) {
	m.drawInventoryItemIconSized(screen, manager, item, x, y, size)
}

// DrawMobileInventoryItemIcon preserves sharp pixel-art scaling on the
// renderer-owned mobile surfaces without changing the desktop filter policy.
func (m *WorldMode) DrawMobileInventoryItemIcon(screen *render.Frame, manager *res.Manager, item session.InventoryItem, x, y, size int) {
	m.drawInventoryItemIconSizedNearest(screen, manager, item, x, y, size)
}
...
// desktop equipment windows and the mobile equipment surface.
func (m *WorldMode) DrawEquipmentPreview(screen *render.Frame, ctx client.Context, x, y, width, height int) {
	m.drawEquipmentPreview(screen, ctx, x, y, width, height)
}

// DrawMobileEquipmentPreview uses the same production character sprite as
// the desktop preview but allows it to occupy the larger mobile paper-doll
// stage instead of retaining the desktop preview scale cap.
func (m *WorldMode) DrawMobileEquipmentPreview(screen *render.Frame, ctx client.Context, x, y, width, height int) {
...
// DrawMobileProfilePreview renders an uncommitted offline appearance through
// the same production humanoid asset lookup used by the world actor.
func (m *WorldMode) DrawMobileProfilePreview(screen *render.Frame, manager *res.Manager, character session.Character, sex byte, x, y, width, height int) {
	if screen == nil || manager == nil || width <= 0 || height <= 0 {
		return
	}
...
func (m *WorldMode) drawHumanoidPreview(screen previewRenderTarget, view *humanoidSpriteView, x, y, width, height int) {
	m.drawHumanoidPreviewScaled(screen, view, x, y, width, height, 1.6)
}

func (m *WorldMode) drawHumanoidPreviewScaled(screen previewRenderTarget, view *humanoidSpriteView, x, y, width, height int, maxScale float64) {
	m.drawHumanoidPreviewScaledWithFilter(screen, view, x, y, width, height, maxScale, spriteDrawFilter())
}

func (m *WorldMode) drawHumanoidPreviewScaledNearest(screen previewRenderTarget, view *humanoidSpriteView, x, y, width, height int, maxScale float64) {
	m.drawHumanoidPreviewScaledWithFilter(screen, view, x, y, width, height, maxScale, spriteCompositionFilter())
}

func (m *WorldMode) drawHumanoidPreviewScaledWithFilter(screen previewRenderTarget, view *humanoidSpriteView, x, y, width, height int, maxScale float64, filter render.Filter) {
	state := spriteState{
	...
	drawBillboardPreview(screen, billboard, x, y, width, height, maxScale, filter)
}

func drawBillboardPreview(screen previewRenderTarget, billboard *spriteBillboard, x, y, width, height int, maxScale float64, filter render.Filter) {
...
func (m *WorldMode) monsterInfoPreviewImage(ctx client.Context, class uint16, width, height int) image.Image {
	...
	target := render.NewImage(width, height)
	drawBillboardPreview(target, billboard, 0, 0, width, height, 1.6, spriteDrawFilter())
	preview := target.RGBA()
